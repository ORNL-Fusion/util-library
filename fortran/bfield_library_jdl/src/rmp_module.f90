...
Module coil_typedef
  ...
  Type, Public :: coil_type
    Real(real64), Allocatable :: coil(:,:)  ! (ncoil_pts,3)
    Real(real64), Allocatable :: current(:)  ! ncoil_pts
    Integer(int32) :: ncoil_pts
  End Type coil_type
...
Contains
  ...
  Subroutine bfield_bs_jdl_1pt(P_x,P_y,P_z,coil,Bx,By,Bz)
    ...
    Ri_x = P_x - coil%coil(1:nfil,1)     !R_i is from beg of filament to point x
    Ri_y = P_y - coil%coil(1:nfil,2)
    Ri_z = P_z - coil%coil(1:nfil,3)
    Ri_mag = Sqrt(Ri_x*Ri_x + Ri_y*Ri_y + Ri_z*Ri_z)
    
    Rf_x(1:nfil-1) = Ri_x(2:nfil)
    Rf_x(nfil) = P_x - coil%coil(nfil+1,1)                    !R_f is from end of filament to x
    Rf_y(1:nfil-1) = Ri_y(2:nfil)
    Rf_y(nfil) = P_y - coil%coil(nfil+1,2)                    !it is thus the same as Ri except one point before
    Rf_z(1:nfil-1) = Ri_z(2:nfil)
    Rf_z(nfil) = P_z - coil%coil(nfil+1,3)
