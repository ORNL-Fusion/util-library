...
Contains
  ...
  !-----------------------------------------------------------------------------
  Subroutine bfield_bs_cyl_1pt(P_r,P_phi,P_z,coil,current,ncoil_pts,Br,Bphi,Bz)
    !
    ! Description:
    ...
    Integer(int32), Intent(in) :: ncoil_pts
    ...
    Call bfield_bs_jdl_1pt(P_x,P_y,P_z,coil,current,ncoil_pts,Bx,By,Bz)
  ...
  !+ Performs Biot-Savart in cylindrical coordinates
  !-----------------------------------------------------------------------------
  Subroutine bfield_bs_cyl_Npts(P_r,P_phi,P_z,npts,coil,current,ncoil_pts,Br,Bphi,Bz)
    !
    ! Description:
    ...
    Call bfield_bs_jdl_Npts(P_x,P_y,P_z,npts,coil,current,ncoil_pts,Bx,By,Bz)
  ...
  !-----------------------------------------------------------------------------
  Subroutine bfield_bs_jdl_1pt(P_x,P_y,P_z,coil,current,ncoil_pts,Bx,By,Bz)
    !
    ! Description:
    ...
    Ri_x = P_x - coil(1:nfil,1)     !R_i is from beg of filament to point x
    Ri_y = P_y - coil(1:nfil,2)
    Ri_z = P_z - coil(1:nfil,3)
    Ri_mag = Sqrt(Ri_x*Ri_x + Ri_y*Ri_y + Ri_z*Ri_z)
    
    Rf_x(1:nfil-1) = Ri_x(2:nfil)
    Rf_x(nfil) = P_x - coil(nfil+1,1)                    !R_f is from end of filament to x
    Rf_y(1:nfil-1) = Ri_y(2:nfil)
    Rf_y(nfil) = P_y - coil(nfil+1,2)                    !it is thus the same as Ri except one point before
    Rf_z(1:nfil-1) = Ri_z(2:nfil)
    Rf_z(nfil) = P_z - coil(nfil+1,3)
  ...
  !+ Performs Biot-Savart in Cartesian coordinates
  !-----------------------------------------------------------------------------
  Subroutine bfield_bs_jdl_Npts(P_x,P_y,P_z,npts,coil,current,ncoil_pts,Bx,By,Bz)
    !
    ! Description:
    !  Performs Biot-Savart evaluation at a set of points given a series of 
    !  current filaments. Points are in Cartesian coordinates (m,radians),
    ...
    Do i = 1,npts
      Call bfield_bs_jdl_1pt(P_x(i),P_y(i),P_z(i),coil,current,ncoil_pts,Bx(i),By(i),Bz(i))
    Enddo
